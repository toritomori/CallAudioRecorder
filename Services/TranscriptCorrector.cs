...
public static class TranscriptCorrector
{
    private const string SystemPromptRu =
        ...
        2. Названия продуктов, компаний и аббревиатуры записывай латиницей в правильном написании
           (например: «эйпиай» → «API», «кубернетес» → «Kubernetes», «гитхаб» → «GitHub»).
           Заимствования, которые склоняются по-русски, оставляй кириллицей и только исправляй
           написание: «ре фактор инг» → «рефакторинг», «за деплоить» → «задеплоить»,
           «в апдейте» остаётся кириллицей.
           Если слово есть в списке терминов в обоих написаниях, выбирай по этому же правилу.
