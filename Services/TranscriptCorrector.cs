...
public static class TranscriptCorrector
{
        ...
        string glossary, IProgress<int>? progress = null, CancellationToken ct = default)
    {
        ...
        {
            user.AppendLine("Термины, имена и названия, которые встречаются в этой встрече " +
                            "(используй правильное написание из списка):");
            user.AppendLine(glossary.Trim());
            user.AppendLine();
        }
        user.AppendLine("Исправь реплики:");
        for (int i = 0; i < entries.Count; i++)
            user.AppendLine($"[{i}] {entries[i].Text}");

        var answer = new StringBuilder();
        await foreach (var chunk in ollama.ChatStreamAsync(model, SystemPrompt, user.ToString(), ct))
        {
            answer.Append(chunk);
            progress?.Report(answer.Length);
        }

        return ApplyCorrections(entries, answer.ToString());
    }
